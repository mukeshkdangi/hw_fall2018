    ...
    private List<EventDetails> processResponse(String response) {
        try {
            ...
            for (int idx = 0; idx < eventArray.length(); idx++) {
                ...
                String date = eventArray.getJSONObject(idx).getJSONObject("dates").getJSONObject("start").optString("localDate");
                String time = eventArray.getJSONObject(idx).getJSONObject("dates").getJSONObject("start").optString("localTime");
                if (Objects.nonNull(time) && !time.isEmpty() && !time.equalsIgnoreCase("null")) {
                    date = date + " " + time;
                }
